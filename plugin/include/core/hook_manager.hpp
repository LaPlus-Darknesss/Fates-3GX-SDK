...
private:
    ...
    static CTRPluginFramework::Hook sHooks[static_cast<std::size_t>(HookId_Count)];

    // Track whether Init() has been called to avoid double
    // initialisation.
    static bool sInitialised;

    // Allow the free helper to access sHooks.
    friend CTRPluginFramework::Hook &GetHookRef(HookId id);
};
